/**
 * Shared test helpers for the PiAgentRunner adapter tests: a fake child
 * pi process (scripted stdout lines / exit) and a spawn factory that
 * records invocations.
 */

import type { PiChildProcess, PiSpawn } from "../../runner/types.ts";
...
export function makeFakeSpawn(): FakeSpawnHandle {
	const handle: FakeSpawnHandle = {
		spawn: (command, args, opts) => {
			...
			handle.records.push({ command, args, cwd: opts.cwd });
...
/** Waits until the spawn factory has produced a child at `index` (async spawn). */
...
/** JSON message_end event helper (pi stream format). */
export function messageEndLine(role: string, overrides: Record<string, unknown> = {}): string {
