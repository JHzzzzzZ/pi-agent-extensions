...
| 3.15 | 成员行连接符 `├─ `（非末项）/ `╰─ `（末项圆角）；上游 fleet-status 用 `├─ `/`└─ `（末项方角，`fleet-status.ts:811/874`） | **差异保留（用户要求圆角）** | agent-team v1.15.3 起成员树连接符改 box-drawing（旧 ASCII `|- ` 退役）：末项取圆角 `╰─` 而非上游方角 `└─`。前缀显示宽度恒 3 列（`├`/`╰`/`─` + 空格，与旧 `|- ` 等宽）——truncateVisible 预算、main/leader 行 `  ` gutter 与选中行 `▸ ` 前缀均不变；末项判定基于 `buildWidgetView` 的行序（main → leader → 成员…，最后一个成员行才是末项；单一成员即末项）。见 `widget.ts` `memberBranch`/`memberRowText`。 |
