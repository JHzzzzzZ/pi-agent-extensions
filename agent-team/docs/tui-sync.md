...
| 模型行文本 | `模型: <model> · 思考 <level>`；model 已知而 level 未知 → `模型: <model> · 思考 （默认）`；model 与 level 均未知 → `模型: （默认）`（无选中成员 → `模型: （无成员）`） | agent-team 特有（v1.16.0，差异条目 §3.15）；`viewer.ts` `modelHeaderText` |
| 思考级别后缀 | `off` / `minimal` / `low` / `medium` / `high` / `xhigh` / `max`（宿主 `VALID_THINKING_LEVELS`；仅剥模型串**最后一个** `:` 且后缀在表内，否则原样；空值/裸后缀 `:level` 不剥） | 宿主 `VALID_THINKING_LEVELS`；`config.ts` `splitModelThinking`（v1.16.0） |
...
| agent-team 1.16.0 | 2026-09-11 | viewer 模型行带思考级别 + live 成员模型升级实际值：`模型: <model> · 思考 <level>`（实际 provider 级别优先、声明模型尾缀回退、未知降级 `（默认）`）；`providerThinkingLevel` 经 usage 通路透传（runner → cockpit live/终态 → viewer / `team_transcript`）；live 成员模型改为 dispatch 实际上报值（`MemberProgress.model` 覆盖声明值）且 `/team:status` 运行态成员行带模型；`config.ts` `splitModelThinking` 仅剥宿主有效级别尾缀；差异条目 §3.15 + §4 字面量；截图重生成；config/runner/cockpit/viewer/viewer-host/run-tool 共 +8 测试（376 全绿 + typecheck 零错误） | `c7f4120` |
