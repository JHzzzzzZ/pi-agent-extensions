...
| 3.18 | `Run:` 行在续跑 run 上追加 `（续跑自 <parentRunId>）`（如 `Run: run-123（续跑自 run-456）`；无 parentRunId 时不变） | **特有语义（fleet 无续跑概念）** | v1.17.0 续跑功能的一部分：`ViewerData.parentRunId` 由 `buildViewerData` 从 `RunProgress.parentRunId` / 终态 `record.parentRunId` 装配，`detailHeaderLines` 单行拼接——**头部仍固定 4 行、帧总行数不变量不变、无键位/焦点变化**；`/team:status` 同口径（runId 行 + failed/aborted 的续跑提示行）。见 `viewer.ts` `detailHeaderLines` + `cockpit.ts` `formatStatusSnapshot`。 |
...
| Run 行文本 | `Run: <runId>`；续跑 run → `Run: <runId>（续跑自 <parentRunId>）` | agent-team 特有（v1.17.0，差异条目 §3.18）；`viewer.ts` `detailHeaderLines` |
...
| agent-team 1.17.0 | 2026-09-11 | `Run:` 行续跑 lineage（差异条目 §3.18）：续跑 run 显示 `Run: <runId>（续跑自 <parentRunId>）`（`ViewerData.parentRunId` 由 `buildViewerData` 装配）；头部仍 4 行、帧高不变量与键位不变。同期续跑功能：leader 会话落盘（`--session-dir`）/原地续写（`--session`）、`team_resume`/`/team:resume`、模型覆盖、worktree 复用——renderer 路径仅上述一行变化；viewer 测试锁定 lineage 且帧行数不变 | `feat/agent-team-resume` |
