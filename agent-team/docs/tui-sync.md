...
| Run 行文本 | `Run: <runId>`；续跑 run → `Run: <runId>（续跑自 <parentRunId>）` | agent-team 特有（v1.20.0，差异条目 §3.20）；`viewer.ts` `detailHeaderLines` |
...
| 问答块文本 | question：`❓ 提问 · <ts>` + 正文换行；answer：`✔ 回答 · <ts>` + 正文换行；无 ts 时省略 ` · <ts>`；未获回答 = `system` 条目（`未获回答（超时）` / `（用户取消）` / `（主会话无 UI）`） | agent-team 特有（v1.18.0，差异条目 §3.19）；`viewer.ts` `blockLines` + `ask.ts` `outcomeEntryText`；提问等待期 leader activity = `等待人工回答：<压平 ≤80 列>` |
