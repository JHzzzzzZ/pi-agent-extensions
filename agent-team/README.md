...
| `team_resume` 工具 | 让主 agent 续跑并**换模型**：`leaderModel` / `memberModels` 只作用于本次续跑 run（不改团队文件，`provider/id:level` 后缀原样透传）；`instructions` 给 leader 补充本次指示（可选，不该重述任务——上下文在会话里）；默认后台 + followUp，`wait: true` 内联返回 |
...
cd agent-team
npm install
npm test          # node --test test/*.test.ts（438 个测试，含真实 git worktree 测试）
