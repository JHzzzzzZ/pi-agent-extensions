...
| 方式 | 说明 |
|---|---|
| `/team <团队名> <任务>` | **后台运行**：首 token 非保留词即视为团队名（参数路由，无需注册、团队增删即时生效）；命令立即返回，主会话可继续对话；输入栏下方亮块实时显示进度（见 §4），完成后报告自动送入会话 |
| `/team run <团队名> <任务>` | 显式派单形态；团队名撞保留词（`run`/`status`/`stop`/`view`/`clear`/`doctor`）时用这个 |
...
其它命令（全部挂在单一 `/team` 命令下）：`/team` 列出全部团队（含无效文件警告，撞保留词的团队会提示显式 `run` 形态）；`/team status` 查看当前/最近一次 run 的详细快照（含 runId，每个成员在做什么、轮次、费用、worktree、预算消耗）；`/team stop` 中止当前 run（SIGTERM → SIGKILL 逐级终止 leader 与成员）；`/team view` **全屏会话记录查看器**（见下节）；`/team clear` 清除输入栏下方的 run 亮块（见 §4）；`/team doctor` **自检报告**（运行模式/团队发现/逐团队模型预检/运行目录残留/逐团队预算/worktree 可用性）。
...
- 命令：单一 `/team` + 子命令 `run`/`status`/`stop`/`view`（内含 `m` 发消息直接对话）/`clear`/`doctor`；`/team <团队名> <任务>` 参数路由派单
...
cd agent-team
npm install
npm test          # node --test test/*.test.ts（299 个测试，含真实 git worktree 测试）
