 ...
 * 落盘——并发/中断边界在 test/{lock,concurrency,interrupt}.test.ts 用真实进程覆盖，
 * 仓库根发现（--root / git）在 test/root-discovery.test.ts 覆盖。本文件只在临时 fixture
 * 目录演练写操作，不触碰仓库真实 todos/；末尾的进程边界 E2E 只跑只读命令。
 */
...
test("CLI E2E：--help 在非仓库 cwd 也退出 0 且含完整用法（migrate 替代 db）", () => {
