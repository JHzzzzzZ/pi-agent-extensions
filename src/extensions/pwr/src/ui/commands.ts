/**
 * PWR UI - /workflow command parsing and control dispatch (JHL-15)
 *
 ...
 * as a command (PRD 4.3: 键盘和命令均须可达). 命令面归一（v2.9.0）：单一
 * `/workflow:*` 命名空间——裸 `/workflow` 是生成入口（`<任务>`）与帮助
 * （空参 / `help`），其余动作全是独立静态子命令：
 *
 *   /workflow:run <name> [args]       - run a saved workflow
 *   /workflow:delete [name]           - delete a saved workflow
 *   /workflow:model [auto|<model-id>] - workflow default model
 *   /workflow:list [status]           - run list (optionally filtered)
 *   /workflow:view [runId]            - full-screen live viewer (JHL-18)
 *   /workflow:open <runId>            - detail
 *   /workflow:pause <runId>
 *   /workflow:resume <runId>
 *   /workflow:stop <runId> [taskId]
 *   /workflow:restart <runId> <taskId>
 *   /workflow:save <runId>
 *   /workflow:saved                   - saved workflow list
...
export function isRunStatus(value: string): value is RunStatus {
	return VALID_RUN_STATUSES.has(value as RunStatus);
}
...
export function parseRunRefArgs(
	...
	if (!runId) return { ok: false, usage: `Usage: /workflow:${command} <runId>` };
...
export function parseControlArgs(
	...
	const usage: Record<string, string> = {
		pause: "Usage: /workflow:pause <runId>",
		resume: "Usage: /workflow:resume <runId>",
		stop: "Usage: /workflow:stop <runId> [taskId]",
		restart_agent: "Usage: /workflow:restart <runId> <taskId>",
	};
...
/**
 * Default run for /workflow:view with no argument: the most recent
...
/**
 * /workflow:approve — manual approval path for runs parked in
...
export function workflowHelpText(): string {
	return [
		"PWR colon command surface (single /workflow namespace):",
		"",
		"Generate & define:",
		...
		"  /workflow:save <runId>              save a run as a reusable command",
		"  /workflow:saved                     list saved workflows (scope, description, args hint)",
		"  /workflow:delete [name]             delete a saved workflow (no name = list them)",
		"  /workflow:model [auto|<model-id>]   workflow default model (no args = show)",
		"",
		"Observe & control runs:",
		"  /workflow:list [status]             list runs (status: draft|awaiting_approval|queued|running|paused|completed|failed|cancelled)",
		"  /workflow:view [runId]              full-screen live viewer (roster + detail)",
		"  /workflow:open <runId>              run detail",
		"  /workflow:pause <runId>             pause run",
		"  /workflow:resume <runId>            resume run",
		"  /workflow:stop <runId> [taskId]     stop run (or one agent)",
		"  /workflow:restart <runId> <taskId>  restart agent (completed cache unchanged)",
		"  /workflow:script <runId>            view raw script (read-only)",
		"  /workflow:approve <runId>           approve a run waiting for approval",
		"  /workflow:help                      this help",
		"",
