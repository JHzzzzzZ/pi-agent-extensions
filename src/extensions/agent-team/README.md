...
**同一成员多轮追问（对话线前文注入，#4）**：对**同一个成员**连续追问属于同一条对话线，不必每轮从零开始。每轮派单把该线**最近 3 轮**的提问 + 该成员答复摘录（各 ≤800 字节）+ 摘要头（轮号 / 已完成轮数 / 注入轮数 / 累计用量）拼进本轮 task，并交代 leader 连同本轮消息一并**原文转派**给成员——成员仍是一次性子进程（不去 `--no-session`、不引入常驻进程、不做成员间旁路），leader 保持唯一中枢；答复摘录取该轮 run 转录里最后一条 assistant，**不**取整段尾部（否则上一轮注入的前文会顺着 `task` 行递归卷进下一轮）。路由复用既有队列与目标页签（不新造派单通道）——所以「怎么发」不变：选成员按 `m` 发送即可，轮与轮之间该排队就排队。

**成本特征（重要）**：每轮仍是一个**独立 run**，因此① 该轮用量落该轮 run 转录（成员收尾行与 `第 N 轮完成 · $x · y tokens` 结局行），并计入该轮 run 的**既有预算**（团队 `budget:` 块的 dispatch / 成员运行次数与可选费用/token 上限，不新增预算类型）——超限照旧 `BUDGET_EXCEEDED` 中止；② 注入头里的「累计用量」只是各轮用量求和展示（按 run 记录口径：leader 累计 + 成员派发），**不是**跨轮新预算；③ 单轮 prompt 成本随对话线上限增长但有界：最近 3 轮 ×（提问原文 + 答复 ≤800 字节）+ 上一 run 尾部 ≤2000 字节；④ 终止沿用 `/team:stop`、viewer `D` 与超时语义（排队消息照旧丢弃）。对话线本身驻内存不落盘（`/reload` 丢线可接受：下一轮从零开线），交互面不变（overlay 输入框 + 主会话，无新增 UI）。
...
- **短题走宿主对话框，长题走可滚动全文视图（v1.27.0）**：题面压平后不超过 200 字符、无空行分段、选项不超过 5 个且每项不超过 60 字符时，仍以**宿主对话框**呈现（与主 agent 其它对话框一致）；超过任一条件（长方案、多段背景、选项多而长）改为**自绘全屏视图**：题面按显示宽度折行全文呈现（`J`/`K` 逐行、`PgUp`/`PgDn` 翻页），选项窗口跟随选中项（`↑`/`↓` 选、`Enter` 提交、`Esc` 取消），不再出现「题面把选项顶出屏幕只能盲选」或「方案被截成前 300 字」。
- **题面长度口径**：题面不再压平成单行、也不再硬截 300 字符（多段结构原样保留）；上限 4KB（UTF-8），超限时在题面尾部**显式标注**`…（题面过长，已省略 N 字节）`，绝不静默藏掉内容。宿主对话框装不下的题面会被静默裁行且无滚动（宿主行为，扩展改不了），所以长题才必须换自绘视图。
- 提问以**宿主对话框（短题）或自绘 overlay（长题）**直接呈现给用户；作答经 leader 的 RPC stdin 回写，leader 带着答案继续任务。
- **与查看器互斥（viewer 盖住对话框修复）**：提问到达时若 `/team:view` 查看器正开着，先**程序化收起查看器**再弹对话框/自绘视图（否则被 overlay 盖住、按键也被选择器抢走），作答/取消/超时后自动重开并回到原成员；收起等待上限 1.5s，异常路径也不会让提问挂起。
- 自绘不可用时（主会话非交互 TUI、宿主 `custom` 不可用）：回退宿主对话框，题面换成前 120 字符摘要 + 「全文见 `/team:view` 的『提问：』块」——全文已先落 run 转录，不丢内容。
- 降级（全部 fail-closed，不会挂住 run）：超时 / 用户取消 / 主会话无 UI（如 headless）/ run 被停止 —— 均回「未获回答」，工具结果要求 leader 不追问、按最合理假设继续并在最终报告说明假设。自绘视图超时以静态文案 `超时：N 分钟后自动取消` 呈现（视图内不放每秒跳动文本）。
...
### 7. 外部 CLI 后端成员（codex / claude）

成员可声明 `backend: codex` 或 `backend: claude`，改用对应 CLI 的非交互模式执行；未声明 backend 的成员仍走 pi 子进程（行为与旧版逐字一致）。外部成员与 pi 成员可在同一团队、同一次 `team_dispatch` 中混编，同 4 并发池、同停止语义（SIGTERM→SIGKILL）、同 50KB/8KB 结果上限，usage/结果折回既有 run 记录、viewer、`/team:status` 与报告 followUp 链路。
