...
| 3.11 | 展开态：`main → leader（含任务摘要）→ 成员…` 树（v1.13.0；任务行 v1.13.1 并入 leader 行）+ 底部提示行；成员连接符 `├─ `（非末项）/ `╰─ `（末项，圆角；v1.15.3） | **特有语义保留** | fleet 无 main 树/任务摘要；agent-team 展开块 = 树行（`main` / `leader <团队> · <任务摘要> ▶ running · 耗时 · N/M 并行[ · 剩 $X.XX]` / `├─ <成员> <图标> <状态>[ · 尾部]`（非末项）/ `╰─ <成员> <图标> <状态>[ · 尾部]`（末项））+ 底部 `↑↓ 选择 · enter 查看 · esc 退出`（无空行、无缩进，`renderWidgetView`）。每行带背景（普通行 `rowBg` = theme `userMessageBg`、选中行 `rowSelectedBg` = theme `selectedBg`，v1.15.3；差异表 §3.16）。**末行恒为成员行**（不设独立任务行：避免用户把末行任务当成员、enter 却打开 leader 的假成员陷阱，用户 2026-09-15 真机反馈）；`main` 行 enter 只收起选中（fleet main 同构），leader/成员行 enter 进查看器（按 actor 钉选）。折叠默认形态对齐 fleet 折叠单行（§2）。历史截图（多行常显/折叠）存 `agent-team/docs/assets/widget-before-collapse.png`；任务行陷阱截图存 `agent-team/docs/assets/widget-tree-feedback.png`。 |
...
| 3.15 | 成员行连接符 `├─ `（非末项）/ `╰─ `（末项圆角）；上游 fleet-status 用 `├─ `/`└─ `（末项方角，`fleet-status.ts:811/874`） | **差异保留（用户要求圆角）** | agent-team v1.15.4 起成员树连接符改 box-drawing（旧 ASCII `|- ` 退役）：末项取圆角 `╰─` 而非上游方角 `└─`。前缀显示宽度恒 3 列（`├`/`╰`/`─` + 空格，与旧 `|- ` 等宽）——truncateVisible 预算、main/leader 行 `  ` gutter 与选中行 `▸ ` 前缀均不变；末项判定基于 `buildWidgetView` 的行序（main → leader → 成员…，最后一个成员行才是末项；单一成员即末项）。见 `widget.ts` `memberBranch`/`memberRowText`。 |
| 3.16 | 亮块每行背景色（普通行 `rowBg` = theme `userMessageBg`、选中行 `rowSelectedBg` = theme `selectedBg`）+ 每行补齐到宿主内容宽（终端宽 − `WIDGET_HOST_INSET = 2`） | **特有语义（fleet-status 无背景块）** | 用户真机反馈：折叠单行与展开各行都要背景色、选中行用更强背景区分。`Styles` 新增 `rowBg`/`rowSelectedBg`（`themeStyles` 经宿主主题取色、取不到色名/无 bg 方法 → 无背景降级）；背景经 ANSI 随 `string[]` 传递（纯字符串 setWidget 路径与接线不变）。宿主对 `string[]` 每行包 `Text(line, 1, 0)`（左右各 1 列 margin 落在背景带外）：本插件按「终端宽 − 2」的 CJK 双宽口径截断+补齐再包背景，背景块每行等宽连续、绝不超出（超宽会折行成额外物理行/触发宿主断言崩溃）。折叠行、树行、折叠提示行、底部提示行同带普通背景；选中行带更强背景。见 `widget.ts` `WIDGET_HOST_INSET`/`renderWidgetView` + `viewer.ts` `Styles`。 |
| 3.17 | detail 模型行追加思考级别段：`模型: <model> · 思考 <level>`——level 取子进程 `message_end.providerThinkingLevel` 实际值，缺省回退团队文件模型尾缀 `provider/id:level`；model 已知而 level 未知 → `· 思考 （默认）`，两者均未知 → 仅 `（默认）` | **特有语义（fleet 检查器无模型行）** | 用户 2026-09-11 需求：view 界面显示 thinking level。管线骑现有 usage 通路（runner 折入 `usage.thinkingLevel` + message_end 事件携带 → cockpit 播种/折入 live progress、finalize 落 `record.leaderThinkingLevel` → `buildViewerData` 装配 `ViewerActor.thinkingLevel` → viewer / `team_transcript` 展示）；声明尾缀经 `config.ts` `splitModelThinking` 按宿主 `VALID_THINKING_LEVELS`（off/minimal/low/medium/high/xhigh/max）剥离。头仍固定 4 行、帧总行数恒为 `bodyHeight + VIEWER_CHROME_ROWS`，键位/焦点语义不变。见 `viewer.ts` `modelHeaderText`/`detailHeaderLines` + `index.ts` `buildViewerData`。 |
| 3.18 | viewer detail 元信息头第 5 行 `活动:`（Run/State/成员/模型/活动）——选中 actor 当前活动：`思考中` / `工具调用 <tool>` / `排队中` / `已完成`/`失败`/`已中止` / `run 已结束`；活动已知时附 ` · 距上次输出 <age>`（5 秒分桶，`0s`/`5s`…，≥60s `2m5s`）。live 阶段由子进程事件驱动（leader：cockpit onEvent 的 tool start/update/end、assistant message_end；成员：dispatch onEvent → `team_dispatch` update 的 details.members → cockpit 折入），无 live progress 时从 transcript 末条推导（末条 tool → 工具调用 + 首 token 工具名；其余 → 思考中；`ts` 供 age） | **特有语义（fleet 检查器无活动行）** | 用户 2026-09-11 需求：viewer 缺运行状态可见性（长时间无输出不可辨认）。**重影约束**：overlay 时钟类文本是堆叠触发器（`docs/incidents.md` 三轮事故）——age 必须 5s 分桶后才烘焙进 `ViewerActor.activity`，该文本进 `viewerDataFingerprint`（时钟驱动重绘从每 750ms 降到至多每 5s 一次、仅 running；终态文本 `run 已结束` 恒定、零时钟重绘），未分桶秒值不得进指纹或渲染。头部 4→5 行，帧总行数仍恒为 `bodyHeight + VIEWER_CHROME_ROWS`，键位/焦点不变。见 `viewer.ts` `formatActorActivity`/`activityFromTranscript`/`ACTIVITY_BUCKET_MS` + `index.ts` `buildViewerData`。 |
...
| 3.23 | **用户输入（viewer `m` 发的消息）在转录里独立成块**：转录新增 `user` kind（落目标 actor + leader 各一条**原文**，不带 wire 标记）；viewer 首行 `▌用户 · <ts>`（accent）+ 正文整行铺 `userMessageBg` 背景（与 agent/leader 输出视觉区分，按 `ts` 插入 body 流）；`team_transcript` 纯文本转储以 `[user] <ts>` 行首标记 | **特有语义（fleet 检查器无用户输入块）** | 用户 2026-09-11 真机反馈（#56）：run 运行中的插话/排队消息在 UI 只剩一句 notice，原文在 viewer 与转录里都查不到。提交时刻落盘（steer / 排队 / 立即派单统一），排队条目的结局补 `system` 行（`已派出（新 run <id>）` / `未派出（run 已停止）` / `未派出（队列已清空）` / `未派出（派出失败）`）——排队中不建第二个事实源（队列本身仍易失不落盘）；notice 文案不变（用户决议）。区块模型/帧几何/键位/焦点不变，仅新增一种内容块。见 `transcript.ts` `TRANSCRIPT_ENTRY_KINDS` + `viewer.ts` `buildBlocks`/`blockLines`（`frame`/`text` 两种渲染模式）+ `chat.ts` `appendEntry` 端口与 `index.ts` 接线；真实宿主渲染锁在 `test/viewer-user-host.test.ts`，三路径锁在 `test/chat.test.ts` + `test/viewer-chat-host.test.ts`。 |
...
| widget 树行 | `main` / `leader <团队> · <任务摘要> ▶ running · <耗时> · <N>/<M> 并行[ · 剩 $X.XX]` / `├─ <成员名> <图标> <状态>[ · <尾部>]`（非末项）/ `╰─ <成员名> <图标> <状态>[ · <尾部>]`（末项，圆角）（无独立任务行；末行恒为成员行） | agent-team 特有（v1.13.0；任务摘要 v1.13.1 并入 leader 行；连接符 v1.15.3）；成员图标 queued `·` / running `●` / done `✓` / failed `✗` / aborted `⊘`；行文本不带 gutter（渲染器统一加 `▸ `/`  `） |
| widget 成员连接符 | 非末项 `├─ `、末项 `╰─ `；前缀显示宽度恒 3 列（与旧 `|- ` 等宽，CJK 双宽口径） | agent-team 特有（v1.15.3；上游 fleet-status.ts:811/874 末项用方角 `└─`，见差异表 §3.15） |
| widget 背景块 | 每行补齐到宿主内容宽（终端宽 − `WIDGET_HOST_INSET = 2`；宿主 `setExtensionWidget` 对 `string[]` 每行包 `Text(line, 1, 0)`，测试直读宿主 dist 源码）后包背景：普通行 `rowBg`（theme `userMessageBg`）、选中行 `rowSelectedBg`（theme `selectedBg`）；折叠行/树行/折叠提示行/底部提示行同带普通背景；取不到色名 → 无背景降级 | agent-team 特有（v1.15.3，差异表 §3.16）；`widget.ts` `renderWidgetView`/`WIDGET_HOST_INSET` + `viewer.ts` `Styles`；真实渲染锚点由 capture-screens 测试锁定 |
| 模型行文本 | `模型: <model> · 思考 <level>`；model 已知而 level 未知 → `模型: <model> · 思考 （默认）`；model 与 level 均未知 → `模型: （默认）`（无选中成员 → `模型: （无成员）`） | agent-team 特有（v1.16.0，差异条目 §3.17）；`viewer.ts` `modelHeaderText` |
| Run 行文本 | `Run: <runId>`；续跑 run → `Run: <runId>（续跑自 <parentRunId>）` | agent-team 特有（v1.21.0，差异条目 §3.20）；`viewer.ts` `detailHeaderLines` |
...
| 用户输入块文本 | 帧：`▌用户 · <ts>` + 正文行（每行补齐到右栏宽后包 `userMessageBg` 背景；无 ts 时省略 ` · <ts>`）；纯文本转储（`team_transcript`）：`[user] <ts>` + 正文行（不铺底） | agent-team 特有（v1.28.0，差异条目 §3.23）；`viewer.ts` `blockLines`（`BlockRenderMode` `frame`/`text`）+ `chat.ts` |
...
| agent-team 1.15.4 | 2026-09-15 | widget 亮块外观（L136+L137）：成员行连接符圆角化 `├─ `/`╰─ `（前缀 3 列等宽，上游方角 `└─`，差异 §3.15）；亮块背景高亮——`Styles` 新增 `rowBg`/`rowSelectedBg`（theme `userMessageBg`/`selectedBg`，缺失降级无背景），每行按宿主内容宽（终端宽 − `WIDGET_HOST_INSET = 2`）CJK 双宽截断+补齐后包背景（等宽连续、不折行不超宽），选中行更强背景（差异 §3.16）。widget 测试新增连接符规则/背景记录端口/宽度补齐/themeStyles 映射；capture-screens 真实宿主 Text 包装测试锁背景连续（120 列 5 物理行 / 20 列窄宽）与折叠态；tui-sync §4 新增连接符/背景/宿主内容宽字面量 | `team/run-1789104779153` |
| agent-team 1.16.0 | 2026-09-11 | viewer 模型行带思考级别 + live 成员模型升级实际值：`模型: <model> · 思考 <level>`（实际 provider 级别优先、声明模型尾缀回退、未知降级 `（默认）`）；`providerThinkingLevel` 经 usage 通路透传（runner → cockpit live/终态 → viewer / `team_transcript`）；live 成员模型改为 dispatch 实际上报值并与声明 provider 前缀组合（`MemberProgress.model` 覆盖归一，口径与终态一致）且 `/team:status` 运行态成员行带模型；`config.ts` `splitModelThinking` 仅剥宿主有效级别尾缀；差异条目 §3.17 + §4 字面量；截图重生成；config/runner/cockpit/viewer/viewer-host/run-tool 共 +8 测试（405 全绿 + typecheck 零错误） | `c7f4120` |
...
| agent-team 1.28.0 | 2026-09-15 | 差异条目 §3.23：用户输入（viewer `m` 发的消息）在转录与 viewer 里可见——转录新增 `user` kind（提交时刻落目标 actor + leader 各一条**原文**，不带 `【用户消息】` wire 标记，steer/排队/立即派单同口径）；viewer 首行 `▌用户 · <ts>`（accent）+ 正文整行铺 `userMessageBg`（`Styles.bubble` 端口，与 agent/leader 输出视觉区分），`team_transcript` 转储带 `[user] <ts>` 标记；排队条目结局补 `system` 行（已派出/未派出——派出失败、run 已停止、队列已清空），排队中不建第二个事实源。`chat.ts` 新增 `appendEntry` 端口 + `index.ts` 接线；新增真实宿主渲染锁 `test/viewer-user-host.test.ts`（VT 样式追踪：背景分类/连续等宽）与三路径转录锁（chat / viewer-chat-host / transcript），共 +14 测试（629→643） | `feat/agent-team-steer-input-visible` |
