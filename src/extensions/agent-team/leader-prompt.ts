...
export function buildLeaderSystemPrompt(
  ...
  sections.push(
    [
      ...
      "# 提问工具：team_ask",
      "",
      "遇到需求歧义、需要人类拍板、或影响结果的假设无法自行判断时，用 `team_ask` 向用户提问（阻塞等待，默认超时 10 分钟）。参数：",
      "",
      "```json",
      `{ "question": "<写清上下文与期望的问题>", "options": ["<选项 A>", "<选项 B>"] }`,
      "```",
      "",
      "规则：",
      "- 只问真正影响结果的阻塞性问题；能自行判断的不要问。",
      "- 一次只问一个问题；question 必须自包含（用户看不到你与成员的对话，写清背景、选项含义与影响）。",
      "- 需要用户在离散方案中选择时给 `options`（2~10 项）；需要自由文本时省略 options。",
      "- 未获回答（超时 / 用户取消 / 主会话无 UI）时不要重复追问、不要空等：基于最合理的假设继续任务，并在最终报告中说明该假设。",
      "",
    ].join("\n"),
  );
