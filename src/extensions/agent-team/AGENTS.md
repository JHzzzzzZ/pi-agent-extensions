...
| 停止/终态 | `cockpit.ts` `TeamRunCoordinator.stop()`（同步 abort）/`stopAndSettle()`（有界等待落定返回终态记录）+ `team_stop` 工具（runId 可选三态：恰 1 活跃停它 / 0 活跃提示 / ≥2 活跃 `RUN_ID_REQUIRED`；aborted 记录补全 roster 成员）；三种显式停止（`/team:stop`、`team_stop`、viewer `D`）统一 `chat.clearRun(runId)`——只丢被停 run 的排队条目（#61；`/team:clear` 才清全队列） |
| viewer 发消息 / 用户输入可见 / 对话线 | `chat.ts`（task 模板 + FIFO 队列 + 链式派出 + `appendEntry` 端口：提交时刻落 `user` 原文到目标 actor + leader、排队条目结局补 `system` 行；**对话线 #4**：同一 actor 多轮追问的前文注入 `buildDialogueBlock` / 答复摘录 `dialogueAnswerExcerpt` / 轮次与用量状态 `onRunFinalized(runId, status, usage)`）+ `index.ts` 接线（`FileTranscriptSink` best-effort + `roundAnswer` 回读最后一条 assistant + `roundUsageOf(record)` 传轮用量）+ `transcript.ts` `TRANSCRIPT_ENTRY_KINDS`（含 `user`）+ `viewer.ts` `blockLines`（`▌用户 · <ts>` + `userMessageBg` 整行；`team_transcript` 转储带 `[user]`） |
