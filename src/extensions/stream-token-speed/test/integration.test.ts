...
test("段前缀：最前段无 `│ `；出现更低排序带后加前缀、低带消失后重渲染（docs/cross/status-bar.md）", () => {
  const anchorCalls: Array<string | undefined> = [];
  ...
  try {
    ...
    h.at(0);
    h.controller.onMessageStart(messageStartEvent(m), h.status);
    assert.equal(h.ui.last(), "TTFT —", "唯一段 = 最前，无前导分隔符（行首定格）");

    // 更低排序带出现：本段不再是“最前”，登记表触发重渲染带 `│ `
    writeBand("05:test-anchor", "anchor", anchorWriter);
    assert.equal(anchorCalls.at(-1), "anchor", "最前段自身不带前缀");
    assert.equal(h.ui.last(), "│ TTFT —", "非最前段带 `│ ` 前缀");

    // 更低排序带消失：恢复最前，重渲染回无前缀
    writeBand("05:test-anchor", undefined, anchorWriter);
    assert.equal(h.ui.last(), "TTFT —", "最前段易主后重渲染无前缀");

    // style 包装的是含前缀的完整文本（前缀决策在样式之前）
    const ui = new RecordingStatusPort();
    ...
    writeBand("05:test-anchor", undefined, anchorWriter);
    h.status.setStatus(STATUS_KEY, undefined);
  }
});
