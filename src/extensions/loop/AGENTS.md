...
## OVERVIEW
`/loop` 定时任务：固定间隔/每天定时/每日窗口循环 + 一次性提醒 + `--bg` 后台 agent（同一任务多轮可重叠运行）。

## WHERE TO LOOK
| 任务 | 位置 |
|---|---|
| 调度语法解析 | `parse.ts`（`every 5m`/`daily at 09:00`/`every 1h from 00:00 to 09:00`/`in 30m`/`at 15:00`） |
| 任务状态/快照 | `tasks.ts`（会话条目快照持久化；快照只装运行中轮次，历史在 `loop-run-v1` 条目） |
| agent 工具 | `tools.ts`（`loop_create/list/delete`） |
| 后台拉起 | `runner.ts`（前台 followUp 送达；`--bg` 拉子 `pi --mode json -p`，会话 id 可 `pi --session` 恢复；真机冒烟须传 `piEntry`） |
...
- 后台轮次重叠不设上限；已结束轮次只进 `loop-run-v1` 条目（别塞快照，全量快照会 O(n²) 膨账）—— 见 `docs/extensions/loop.md` 不变量。
...
## COMMANDS
```bash
cd src/extensions/loop && npm install && npm test   # 205 测试；另有 npm run typecheck
node src/extensions/loop/test/bg-overlap-smoke.mjs  # opt-in 真机：两轮并发（需鉴权+网络）
```
