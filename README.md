...
| [`goal/`](#goal) | 会话目标循环：`/goal` 设定条件，agent 跨回合自动推进直至评估器判定达成（清除非阻塞项走 `/goal:*` 冒号子命令） | 61 个 |
...
```bash
node --experimental-strip-types --test goal/index.test.ts goal/aligned-ticker.test.ts   # 61 个测试
```
