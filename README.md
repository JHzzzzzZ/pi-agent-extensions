...
| 扩展 | 作用 | 测试 |
| --- | --- | --- |
| [`pwr/`](#pwr--pi-workflow-runtime-主项目) | 工作流编排：脚本引擎 + 子进程 runner + 批准/保存/UI（solo 开启时批准卡按 once 自动批准；`/workflows` 与 `/workflow run|delete|model` 子命令式命令面） | 417 个（node:test） |
| [`agent-team/`](#agent-team--多-agent-团队协作) | 可复用多 agent 团队：leader 调度成员协同完成任务（含全屏分栏会话记录查看器，支持查看器内停止 run、m 发消息直接对话；单一 `/team` 命令 + 子命令路由） | 288 个 |
...
npm test           # 417 个单测（test/ + tests/ + runtime/test/ + runner/test/）
...
```bash
cd agent-team
npm install && npm test        # 288 个测试（含真实 git worktree 用例）
npm run typecheck
```
