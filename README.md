...
| [`agent-team/`](#agent-team--多-agent-团队协作) | 可复用多 agent 团队：leader 调度成员协同完成任务（含全屏分栏会话记录查看器，支持查看器内停止 run、m 发消息直接对话；冒号命令面 `/team:list|:run|:status|:stop|:view|:clear|:doctor`） | 331 个 |
...
```bash
cd agent-team
npm install && npm test        # 331 个测试（含真实 git worktree 用例）
npm run typecheck
```
