...
| [`src/extensions/loop/`](#loop) | /loop 定时任务：固定间隔 / 每天定时 / 每日窗口循环 + 一次性提醒 + --bg 后台 agent 模式（可选模型指定；同一任务多轮可重叠；管理走 `/loop:*` 冒号子命令） | 205 个（node:test） |
...
- **派单与复用** — `/team:run <团队> <任务>`（统一派单入口）、`team_run` 工具（默认后台，返回含 runId；报告或失败摘要完成后自动送达主会话——failed 状态/错误/成员结果/部分报告同通道必达，主 agent 可重试或如实转告用户）；同一团队反复使用；`/team:stop [runId]` 或 `team_stop` 工具中止（settle-aware：停止后拿到 aborted 终态记录，报告 followUp 不再送达，可立即重新派单；**只丢弃被停 run 排队的 viewer 对话消息**——其他并行 run 的排队保留，转录结局记「未派出（run 已停止）」，只有 `/team:clear` 清全队列，#61）；团队名可与子命令同名（保留词概念已退役）
...
- **外部成员思考档位（#66）** — 外部成员的 `model:` 与 pi 成员同写法（`model: <id>:<level>`，宿主级别集 off/minimal/low/medium/high/xhigh/max）：后缀从 `--model` 剥出，按各 CLI **实际支持集**注入参数（codex `-c model_reasoning_effort=<level>`：minimal/low/medium/high/xhigh；claude `--effort <level>`：low/medium/high/xhigh/max），不再被当模型名直传，顺带修掉「预检剥后缀、启动不剥」的不一致；两端不支持的档位（如 claude 的 `:off`、codex 的 `:max`）run 预检期 `EXTERNAL_THINKING_UNSUPPORTED` fail-closed（不生成参数、不 spawn、不静默降级），`team_resume` 的 `memberModels` 覆盖同受校验；映射表与实测来源（`claude --help` / codex 官方 config 参考）见 [src/extensions/agent-team/README.md](src/extensions/agent-team/README.md) §7
...
- **成员终态判定（v1.31.0，#47）** — 「交付完成」只看**最后一轮**的结束状态：早轮失败（宿主 auto-retry / 限流重试）不再把已交付完整报告与 commit 的成员判成 `failed`（真机事故：leader 按「环境级失败不重试」丢弃已完成的工作）；末轮干净而进程退出码非 0 判 `done` + 「收尾异常：exit N」（三态不变，产出照常可用但不静默——leader 报告分节、成员转录收尾行、失败通知三处可见）；末轮报错 / 进程被信号杀 / 工具执行中途被打断才是真 `failed`（错误消息必带 exit code 或信号，被打断标注「部分产出（可能可用）」），失败成员已产出的正文与「（已有完整产出 N 字节，可直接取用）」一并进 leader 报告——产出已拿到就不该白重派；诊断字段 `diagnostics`（exitCode / signal / 末轮 stopReason / 前轮错误前 3 条与计数）随成员结果落转录、失败通知与 run 记录；pi 成员与外部 CLI 成员（codex / claude）共用同一判定
...
- **同一成员多轮追问（#4，对话线前文注入）** — 在查看器里对**同一个成员**连续追问不再每轮从零开始：同一（团队，成员）的连续消息属于同一条**对话线**，每轮派单注入**最近 3 轮**的提问 + 该成员答复摘录（各 ≤800 字节）+ 摘要头（轮号/已完成轮数/累计用量），成员仍是一次性子进程（不去 `--no-session`、无常驻进程、无成员间旁路，leader 是唯一中枢，前文由它按指令原文转派）。路由复用既有 chat 队列与目标页签（不新造派单通道）；每轮仍是独立 run——该轮用量落该轮 run 转录（`第 N 轮完成 · $x · y tokens`）并计入该 run 的**既有预算**（dispatch / 成员运行 / 费用三类上限，不新增预算类型，注入头里的累计用量仅展示），超限按既有语义中止；终止沿用 `/team:stop`、viewer `D` 与超时（排队消息按既有语义丢弃）。答复摘录取该轮转录最后一条 assistant（不会把上一轮注入的前文递归卷进下一轮）；交互面不变（overlay 输入框 + 主会话），无新增 UI
...
npm install && npm test        # 688 个测试（含真实 git worktree 与真实 pi 子进程 E2E）
...
| `/loop --bg <上述任意创建形态>` | **后台模式**（v1.3.0）：到期不注入当前会话，而是拉起独立子 pi 进程（`pi --mode json -p --name loop-<id>-<HHMM>`，无 `--no-session`），每次触发开新会话落盘；会话 id 自动记入轮次记录，用 `pi --session <id>` 可随时恢复后台对话记录。**同一任务的多轮允许真实重叠（v1.8.0，不设并发上限）**——单轮耗时超过间隔（如 15m 巡检 + 单轮 40 分钟）时不再丢触发；会话关闭自动终止全部在途轮次并逐轮标记 interrupted |
...
| `/loop:list` | 查看全部任务（后台任务附 `[后台]` 徽标、运行中轮次（每轮一行：起始时刻 + 已跑时长 + 会话 id）与最近 10 条已完成轮次；更早的折成一行计数） |
...
```text
widget：⏰ loop 2 个任务 · 下次 04:32 · 后台运行 2 轮（1 个任务）

/loop:list：
a1b2c3d4  every 30m                          14:30:00  检查 CI 状态
e5f6g7h8  [后台] every 1h from 00:00 to 09:00  01:00:00  夜间巡检部署
└ 完成 · 00:00:05 · 会话 01a0a5aa-… · 全部正常
└ 运行中 · 01:00:05 · 已跑 12m30s · 会话 01a0a5ab-…
└ 运行中 · 01:00:06 · 已跑 12m29s · 会话 01a0a5ac-…
```

daily/window 调度与固定间隔共用同一套语义：错过的时间点不补跑（跨天/跨窗口只触发一次），暂停后恢复、会话恢复（hydrate）时错过的触发点直接重算到下一个未来时刻；旧格式快照（无 schedule 字段）零迁移兼容。

**后台模式细节**（v1.3.0，`runner.ts`；v1.4.0 起支持模型指定，v1.8.0 起允许轮次重叠）：子进程 cwd 取宿主会话目录，会话落在该项目的 sessions 目录（`pi -r` 选择器可见，`--name loop-<id>-<HHMM>` 可辨识）；JSON 输出首行会话头 `{"type":"session","id":…}` 捕获后立即回调写进轮次记录；单次运行超时 3 小时（SIGTERM→SIGKILL；须大于 headless auto-drain 的 30 分钟上限，理由见 `docs/extensions/loop.md`）；完成后通知结果摘要与恢复提示。
...
node .agents/skills/todo-cli/todo-cli/todo.mjs add --file <name> "描述" [--tag 词1,词2] [--dep 文件#id,...]   # 追加登记（跨文件查重，重复拒绝；--force 强制；--dep 登记即声明依赖）
...
node .agents/skills/todo-cli/todo-cli/todo.mjs complete --file <name> --match "子串" [--note "说明"]  # 完成：status→done，--note 逐字进 notes（aligning/aligned 收口必带 --note）；输出直接依赖者提示
node .agents/skills/todo-cli/todo-cli/todo.mjs reopen --file <name> --match "子串" [--note "原因"]    # 撤销：在途条目→open（清 branch/claimedAt/alignedAt；aligning/aligned 必带 --note；done 拒绝；对齐文档归档）
