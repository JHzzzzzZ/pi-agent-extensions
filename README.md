...
| [`agent-team/`](#agent-team--多-agent-团队协作) | 可复用多 agent 团队：leader 调度成员协同完成任务（含全屏分栏会话记录查看器，支持查看器内停止 run、m 发消息直接对话；输入栏下方可选中亮块，展开为 main→leader→成员树，大团队自动窗口化；冒号命令面 `/team:list|:run|:status|:stop|:view|:clear|:doctor`） | 366 个 |
...
- **隔离与统计** — 成员可选 `worktree: true` 独立 git worktree（分支 `team/<runId>/<member>`，不自动合并）；同一 run 重派对同一成员复用已注册的 worktree/分支（空闲同名分支 attach 复用），失败文案穿透 git 进度行取真 fatal；按成员统计 token/费用；运行记录持久化为会话 entry
...
```bash
cd agent-team
npm install && npm test        # 366 个测试（含真实 git worktree 用例）
