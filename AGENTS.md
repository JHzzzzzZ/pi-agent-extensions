...
cd agent-team && npm install && npm test                                        # 290 个测试（node --test test/*.test.ts）
...
D
- **数量（grep 实测）：** pwr 436 个测试，分布在 35 个 `*.test.ts`（test/ 103、tests/ 232、runtime/test/ 56、runner/test/ 45）；stream-token-speed 43；agent-team 290；run-timer 47；loop 182；goal 44；provider-quota 15；opencode-bridge 114；chatanywhere-provider 32；deep-init 37；human-notify 37；solo-mode 11。
