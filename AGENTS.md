...
cd opencode-bridge && npm install && npm test                                    # 109 个测试（helper 集成测试派生真实 helper + 手写 fake SOCKS5）；另有 npm run typecheck
...
D
- **数量（grep 实测）：** pwr 412 个测试，分布在 34 个 `*.test.ts`（test/ 101、tests/ 210、runtime/test/ 56、runner/test/ 45）；stream-token-speed 43；agent-team 279；run-timer 47；loop 182；goal 44；provider-quota 15；opencode-bridge 109；chatanywhere-provider 32；deep-init 32；human-notify 37。
