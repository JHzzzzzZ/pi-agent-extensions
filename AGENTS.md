...
cd agent-team && npm install && npm test                                        # 215 个测试（node --test test/*.test.ts）
...
D
- **数量（grep 实测）：** pwr 405 个测试，分布在 33 个 `*.test.ts`（test/ 100、tests/ 204、runtime/test/ 56、runner/test/ 45）；stream-token-speed 43；agent-team 215；run-timer 47；loop 169；goal 44；provider-quota 15；opencode-bridge 108；chatanywhere-provider 32；deep-init 32；human-notify 32。
