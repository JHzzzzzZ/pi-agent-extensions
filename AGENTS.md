...
cd agent-team && npm install && npm test                                        # 506 个测试（node --test test/*.test.ts；另 node test/resume-host-smoke.mjs 验真实 pi 的 --session 续写）
