...
cd agent-team && npm install && npm test                                        # 267 个测试（node --test test/*.test.ts）
...
cd opencode-bridge && npm install && npm test                                    # 113 个测试（helper 集成测试派生真实 helper + 手写 fake SOCKS5）；另有 npm run typecheck
...
D
- **数量（grep 实测）：** pwr 410 个测试，分布在 35 个 `*.test.ts`（test/ 103、tests/ 206、runtime/test/ 56、runner/test/ 45）；stream-token-speed 43；agent-team 267；run-timer 47；loop 182；goal 44；provider-quota 15；opencode-bridge 113；chatanywhere-provider 32；deep-init 37；human-notify 37；solo-mode 11。
