...
cd agent-team && npm install && npm test                                        # 299 个测试（node --test test/*.test.ts）
...
D
- **数量（grep 实测）：** pwr 437 个测试，分布在 35 个 `*.test.ts`（test/ 103、tests/ 233、runtime/test/ 56、runner/test/ 45）；stream-token-speed 43；agent-team 299；run-timer 59；loop 192；goal 60；provider-quota 25；opencode-bridge 114；chatanywhere-provider 32；deep-init 37；human-notify 37；solo-mode 12；根契约 2。
