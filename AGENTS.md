...
cd opencode-bridge && npm install && npm test                                    # 70 个测试（helper 集成测试派生真实 helper + 手写 fake SOCKS5）；另有 npm run typecheck
cd deep-init && npm install && npm test                                               # 28 个测试（纯函数 + fake scanner）；另有 npm run typecheck
```
...
- 缩进：`pwr/` 用 tab，`agent-team/`、`run-timer/`、`stream-token-speed/`、`loop/`、`goal/`、`opencode-bridge/`、`deep-init/` 用 2 空格。
...
- **数量（grep 实测）：** pwr 405 个测试，分布在 33 个 `*.test.ts`（test/ 100、tests/ 204、runtime/test/ 56、runner/test/ 45）；stream-token-speed 43；agent-team 85；run-timer 47；loop 169；goal 39；provider-quota 15；opencode-bridge 70；deep-init 28。
