...
- **`pwr/` — 主项目。** PWR（Pi Workflow Runtime）v2.6.0：用户编写受约束的 ECMAScript 工作流脚本；PWR 校验后弹出批准卡，再通过派生子 `pi` 进程作为 sub-agent 执行（`PiAgentRunner`）。v2.4.0 新增逐步实时运行 trace（runner `onEvent` → runtime `task_event` → 查看器 agent 行）与 `/workflows saved` 列表，以及 saved 工作流的 schema 引导 `key=value` 参数输入（仍兼容 JSON）；v2.4.2 接线会话生命周期（`session_shutdown` 中止在途 run + `session_start` 复活单例 runtime）；v2.5.0 接 solo 审批门（`/solo` 开启时批准卡按 once 自动批准，绝不写 remembered，`src/solo-gate.ts`）；v2.6.0 命令面统一为子命令式（13 个 `/workflows:*` 合并为单 `/workflows` + 子命令；saved 调用改为 `/workflow run <name> [args]`，动态 `/workflow:<name>` 注册退役，saved 名运行时现读盘）。零构建 TypeScript ESM，由 Node ≥ 22.18 原生 type-stripping 直接执行。
...
D
- **数量（grep 实测）：** pwr 416 个测试，分布在 35 个 `*.test.ts`（test/ 103、tests/ 212、runtime/test/ 56、runner/test/ 45）；stream-token-speed 43；agent-team 279；run-timer 47；loop 182；goal 44；provider-quota 15；opencode-bridge 114；chatanywhere-provider 32；deep-init 37；human-notify 37；solo-mode 11。
