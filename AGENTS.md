...
- `agent-team/` specifics: team = persistent Markdown file (frontmatter `leader` + `members[]` with per-member `provider/model`, `tools`, `worktree`, block-scalar `prompt`) in `~/.pi/agent/teams/` or trusted-project `.pi/teams/` (project wins on name conflicts); discovery re-scans on every use (no cache). One codebase, two modes keyed on env `PI_AGENT_TEAM_FILE`: leader mode registers only the `team_dispatch` tool; cockpit mode registers `team_create`/`team_list`/`team_run` tools, `/team*` commands, the below-editor selectable run widget (`widget.ts`: first `setWidget(key, factory, { placement: "belowEditor" })` user in the workspace — mounted once per session, pulls `getStatus()` on a 1s repaint tick, modal key handling via `tui.addInputListener` so bare arrows/enter stay with the editor until alt+↓/alt+↑ selects; run end switches to a terminal row instead of freezing on running), entry renderer (`agent-team-run-v1`). `team_run` defaults to background dispatch (immediate return; the report arrives as a followUp turn via `deliverRunResult`/`session.ts`), `wait: true` keeps the synchronous tool contract; the extension entry accepts `{ spawn }` for tool-level tests (`test/run-tool.test.ts`). Member/leader children follow the same child-`pi` JSON-mode pattern as pwr's runner (`team-tmp://` prompt materialization, SIGTERM→SIGKILL), self-contained (no pwr imports). Result unions use `TeamErrorCodes`; caps: 8 tasks/dispatch, 4 concurrent members, 50KB result, 8KB summary.
