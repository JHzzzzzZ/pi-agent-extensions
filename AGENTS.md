...
cd agent-team && npm install && npm test                                        # 96 tests (node --test test/*.test.ts)
...
- Indentation: tabs in `pwr/`, 2 spaces in `agent-team/`, `run-timer/`, `stream-token-speed/`, `loop/`, and `goal/`.
- `agent-team/` specifics: team = persistent Markdown file (frontmatter `leader` + `members[]` with per-member `provider/model`, `tools`, `worktree`, block-scalar `prompt`) in `~/.pi/agent/teams/` or trusted-project `.pi/teams/` (project wins on name conflicts); discovery re-scans on every use (no cache). One codebase, two modes keyed on env `PI_AGENT_TEAM_FILE`: leader mode registers only the `team_dispatch` tool; cockpit mode registers `team_create`/`team_list`/`team_run` tools, `/team*` commands, the below-editor selectable run widget (`widget.ts`: `setWidget(key, string[], { placement: "belowEditor" })` on a 1s interval — host-wrapped string widgets are the rendering path proven stable across host builds, a component-factory repaint variant produced appended-line trails on one bundled host; modal key handling via `ctx.ui.onTerminalInput` (feature-detected) so bare arrows/enter stay with the editor until alt+↓/alt+↑ selects; run end switches to a terminal row instead of freezing on running; `PI_AGENT_TEAM_WIDGET=0` disables it), entry renderer (`agent-team-run-v1`). `team_run` defaults to background dispatch (immediate return; the report arrives as a followUp turn via `deliverRunResult`/`session.ts`), `wait: true` keeps the synchronous tool contract; the extension entry accepts `{ spawn }` for tool-level tests (`test/run-tool.test.ts`). Member/leader children follow the same child-`pi` JSON-mode pattern as pwr's runner (`team-tmp://` prompt materialization, SIGTERM→SIGKILL), self-contained (no pwr imports). Result unions use `TeamErrorCodes`; caps: 8 tasks/dispatch, 4 concurrent members, 50KB result, 8KB summary.
...
- **Counts (measured via grep):** pwr 405 tests across 33 `*.test.ts` files (test/ 100, tests/ 204, runtime/test/ 56, runner/test/ 45); stream-token-speed 43; agent-team 96; run-timer 47; loop 169; goal 39; provider-quota 15.
