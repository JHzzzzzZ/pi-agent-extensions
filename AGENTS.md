...
npm run test:todo       # todo CLI 纯逻辑 + 临时目录闭环单测（9 个测试）
node tools/todo.mjs summary    # agentic todo CLI：全量盘点（list/add/claim/complete/lint 见文件头）
...
- **数量（grep 实测）：** pwr 439 个测试，分布在 35 个 `*.test.ts`（test/ 105、tests/ 233、runtime/test/ 56、runner/test/ 45）；stream-token-speed 45；agent-team 354；run-timer 59；loop 193；goal 63；provider-quota 26；opencode-bridge 114；chatanywhere-provider 32；deep-init 37；human-notify 37；solo-mode 22；根契约 3 + 安装冒烟单测 19 + todo CLI 9。
