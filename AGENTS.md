...
- `test/` — 根契约测试（`status-bar-contract.test.ts`：footer 排序带 + 段分隔/首段定格 + widget 栈顺序，`npm run test:contract`）与安装冒烟工具的纯逻辑单测（`install-smoke.test.ts`，`npm run test:smoke`）。
- `tools/` — 仓库级开发工具：`install-smoke.mjs`（全新临时配置目录 + 真实 `pi --mode rpc` 进程，验证 12 个扩展在“手动复制”安装形态下全部加载，`node tools/install-smoke.mjs`）。
...
npm run test:smoke      # 安装冒烟工具纯逻辑单测（8 个测试，不 spawn pi）
...
cd agent-team && npm install && npm test                                        # 339 个测试（node --test test/*.test.ts）
...
- **数量（grep 实测）：** pwr 439 个测试，分布在 35 个 `*.test.ts`（test/ 105、tests/ 233、runtime/test/ 56、runner/test/ 45）；stream-token-speed 45；agent-team 339；run-timer 59；loop 193；goal 63；provider-quota 26；opencode-bridge 114；chatanywhere-provider 32；deep-init 37；human-notify 37；solo-mode 15；根契约 3 + 安装冒烟单测 8。
