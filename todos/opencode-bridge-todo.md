# TODO
- [ ] 命令风格统一（跨插件）：连字符长命令 `/opencode-bridge-sync`、`/opencode-bridge-restore` 与统一基准（子命令式）不一致。跨插件需求，已在 agent-team-todo.md 同步登记。（processing 2026-09-10 @ feat/commands-subcommand-style：合并为单 `/opencode-bridge` + 子命令 `sync [port]`、`restore`，无参仍为状态；旧 `-sync`/`-restore` 不再注册）
