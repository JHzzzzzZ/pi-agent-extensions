...
- [ ] 建立工作区 `docs/` agent 知识库，减少开发时的代码通读。（processing：阶段 A 骨架+样板卡已完成待用户过目，阶段 B 待铺满）
  ...
  - [ ] `docs/extensions/` 其余 9 张卡（chatanywhere-provider / deep-init / goal / human-notify / loop / opencode-bridge / provider-quota / run-timer / stream-token-speed），阶段 B subagent 并行产出 + 主代理硬校审。
  - [ ] 阶段 B 收尾：INDEX 去占位确认、验收抽查、合回主干并删 worktree。
