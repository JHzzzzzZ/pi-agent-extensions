...
- [ ] agent-team 的 TUI 与 pi-subagents 同步，所有细节同步到代码层级

  agent-team 的 viewer/widget 是对照 pi-subagents（当前 v0.66.0，`src/tui` + `src/extension` + fleet 壳）手抄的；两边一旦各改各，显示 bug（如重影堆叠）会反复出现。以后 pi-subagents 的 TUI 每变一次，agent-team 跟进一次，差异只留 agent-team 特有语义。
  - [ ] 建对照矩阵：agent-team 侧（`viewer.ts` / `widget.ts` / cockpit 状态行）逐文件对应到 pi-subagents 侧源文件 + 版本号（基线 v0.66.0），矩阵落盘（如 `agent-team/docs/tui-sync.md` 或 AGENTS.md 小节）。
  - [ ] 同步粒度到代码层：overlay options、maxHeight/margin、刷新节流、键盘交互、open/close 互斥、销毁与重入语义，能一字不差就一字不差；抄不来的差异逐条注释写明原因。
  - [ ] 同步≠依赖：仍不 import pi-subagents 包（自包含要求保留），以“对照抄改 + 单测锁定”方式同步；每次同步记新版本号，进对照矩阵与提交信息。
