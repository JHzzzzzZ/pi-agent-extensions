...
- [ ] 状态条实现方式参考 pi-subagents fleet-status（组件工厂式状态栏，未领取）
  ...
  - 待确认：用户所指是下方 fleet-status 组件式状态区，还是 slash 运行期 footer `setStatus` 提示；范围是替换现有两行亮块 vs 新增独立状态条 key；是否要逐成员行/树/滚动/展开、spinner 与身份色是否保留 agent-team 特有 `alt+↓/↑` 第二通道。
