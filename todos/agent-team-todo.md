...
- [ ] viewer 运行中成员模型升级为「实际值」：当前 live 阶段成员 `模型:` 显示团队文件声明值（`MemberProgress.model`），只有终态优先 `member.usage?.model`（子进程 message_end 实际上报）。runner 已把成员实际模型折进 dispatch 结果 `usage.model`（`runner.ts` `outcome.usage.model = msg.model`，`parseDispatchMemberResults` 已透传 usage），cockpit 收到 `team_dispatch` 结果时把 `progress.members[].model` 覆盖为实际值即可——等待一次真实多模型派单验证声明值与实际值不同时才动手（避免无谓改动）。
