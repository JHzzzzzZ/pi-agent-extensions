...
- [ ] login 时上下键冲突：Pi 的 login（登录/鉴权选择）界面中，team widget 的上下键选中激活与界面的选项上下选择冲突，导致无法选择选项。（细节待补记：具体是 Pi 原生登录页还是对话内 login 命令的选项列表；修复方向可能是 widget 激活门控需识别宿主 overlay/选择器处于打开状态时让行上下键，或仅保留 alt+↓ 通道）
