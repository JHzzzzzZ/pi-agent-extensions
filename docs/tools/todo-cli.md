...
- `test/`（同一 `todo-cli/` 目录内）— `todo-cli.test.ts`(38，命令闭环：两段式 claim / align 门 / complete 收口门 / reopen 回退与归档 / list / summary / lint / triage / globalId 落盘与写门禁 + fail-closed + 5 个只读进程边界 E2E)、`root-discovery.test.ts`(7，`--root`/git/失败路径纯测 + 真实 `git init` 子目录发现 E2E)、`skill.test.ts`(2，SKILL.md frontmatter 与命令面 + 包装器指向)、`schema.test.ts`(9)、`align.test.ts`(6)、`depends.test.ts`(7)、`lock.test.ts`(8)、`query.test.ts`(6)、`migrate.test.ts`(12)、`globalid.test.ts`(6，计数器取号/自愈/损坏/迁移等价纯函数)、`concurrency.test.ts`(5，真子进程并发 add/claim/align/dep + 跨文件并发取号；经 `withFailureScene` 接线失败现场)、`interrupt.test.ts`(1，SIGKILL 轮次 + stale 自愈 + tmp 清理 + 号不重号；同接线)、`failure-scene.ts`（失败现场诊断 helper，不匹配 `*.test.ts` glob 且不进 fixture 拷贝清单）、`failure-scene.test.ts`(9，现场渲染/同一 Error 重抛/碰撞序号/真子进程注入契约)。
...
- 必跑：`npm run test:todo`（glob = `.agents/skills/todo-cli/todo-cli/test/*.test.ts`；116 个，2026-09-16 实测全绿，含 6 个全局 id 计数器用例、7 个依赖图纯函数用例、reopen 回退/归档用例、9 个失败现场 helper 契约用例、迁移与并发/中断真子进程用例）+ `node .agents/skills/todo-cli/todo-cli/todo.mjs lint`（exit 0）；仓库无根级 typecheck 门，新文件全用可擦除 TS 语法。
