...
`DEFAULT_SUITES`（`tools/test-all.mjs` 顶部常量）逐条镜像各插件 README / `package.json` 的测试命令：12 个扩展 + agent-manager + 根三套自检（contract / smoke / todo）+ test-all 自身 = **17 套件**（用例 1920 = 3be25cc 实测 1902 + todo-cli #15 新增 18，`npm run test:todo` 实测 110/110）。
