...
- 亮块 `setWidget` 传**纯字符串数组**（样式以 ANSI 随字符串传递：fg + 行背景由 `themeStyles` 注入，v1.15.3）——`ExtensionUIContext` 无 `theme` 字段，类型化访问 `ctx.ui.theme` 无法编译。
...
- 必跑：`cd agent-team && npm install && npm test`（372 个）+ `npm run typecheck`。
