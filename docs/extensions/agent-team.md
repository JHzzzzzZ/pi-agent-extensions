...
- viewer 右栏元信息头为固定四行 `Run:`/`State:`/`成员:`/`模型:`（v1.14.0；thinking 段 v1.16.0）：模型=选中 actor 的后端——leader 取子进程 `message_end` 实际上报值（live `RunProgress.leaderModel` / 终态 `record.leaderUsage?.model`），成员跑过后取 dispatch 折入的实际上报值（v1.16.0，live `MemberProgress.model` 由 cockpit 启动时先写声明值、dispatch 结果到达即覆盖；终态优先 `member.usage?.model`），未跑过回退团队文件声明值；模型行追加 ` · 思考 <level>`——实际 provider 级别（runner 折入 `usage.thinkingLevel` / message_end 事件携带，cockpit live `RunProgress.leaderThinkingLevel` 与终态 `record.leaderThinkingLevel`）优先，缺省回退声明模型尾缀 `provider/id:level`（仅宿主有效级别 off/minimal/low/medium/high/xhigh/max；`config.ts` `splitModelThinking`），model 已知而级别未知显示 `· 思考 （默认）`、两者均缺省仅 `（默认）`；头部仍不滚、帧总行数恒为 `bodyHeight + VIEWER_CHROME_ROWS`（行数 3→4 只吞一行正文窗口）。viewer 刷新不为此重读团队文件，也不改变任何键位/焦点语义（差异条目 tui-sync §3.13 + §3.15）。
...
- 必跑：`cd agent-team && npm install && npm test`（376 个）+ `npm run typecheck`。
