...
- **末尾静默坑（v3/v4 口径）**：message_end 时末尾 1s 窗口无输出，`computeSummary` 的「最后瞬时」沿用最近一次渲染的非零平滑值并置 `lastInstantCarried`（热身期未完成无种子时退回有效时长平均，恒非零）——但该值**不再上屏**：汇总只显示 TTFT + 平均（`~` 标注平均），`lastInstantTps`/`lastInstantCarried` 仅保留在汇总结构中（metrics.ts `computeSummary` + 集成测试回归）。
