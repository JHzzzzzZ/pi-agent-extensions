...
- 状态键固定 `"50:stream-token-speed"`（status-port.ts `STATUS_KEY`；`50:` 为 footer 排序带，宿主逐行渲染依赖本地补丁 `docs/pi-footer-status-patch.md`，不可改回无前缀键）。不接对齐秒节拍：250ms 流式节流是内容驱动的，不是墙钟时间类状态。
