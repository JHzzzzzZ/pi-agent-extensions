...
- `src/ui/` — 无宿主 TUI 层；`renderer.ts` 是唯一引 pi-tui 组件的文件。footer 状态键为 `30:pwr`（排序带，见 `docs/cross/status-bar.md`）；widget 键 `pwr-runs` 不带前缀。
...
- 必跑：`cd pwr && npm test`（417 个）+ `npm run typecheck`；性能门：`test/perf.test.ts`（1500-agent 脚本校验 ≤300ms）。
