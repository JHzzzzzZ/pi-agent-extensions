...
opencode-go 等模型按出口 IP 限区，Pi 又只支持 HTTP 代理（不认 socks5://）——本扩展在 session_start 拉起/复用一个独立 helper 进程（opencode-bridge-helper.mjs），把 Pi 的 httpProxy 流量经 HTTP CONNECT 转到本地 SOCKS5（v2rayN 默认 127.0.0.1:10808）。**不做**：绝不自动改 settings.json（sync 必须人工确认）、不是开机自启（随 Pi 会话起）、不持有 helper 子进程资源、不代理 Pi 以外的流量。
...
- `index.ts` — Pi 宿主接线：session_start 探测/拉起、单 `/opencode-bridge` 命令 + 子命令（无参=状态、`sync [port]`、`restore`；v1.5.0 合并，旧 `-sync`/`-restore` 不再注册）。
...
2. `/opencode-bridge sync` → planHttpProxySync 只读出 plan（SET / REMOVE / NOOP / FOREIGN）→ ctx.ui.confirm 人工确认 → applyHttpProxySync 备份原文后落盘 → 重启 Pi 才生效。
...
- fail-closed：任何落盘动作（写 httpProxy / 改端口迁移 / 恢复备份）必须经一次人工 confirm 才执行，plan/apply 两阶段（planHttpProxySync / applyHttpProxySync，bridge.ts）——绝无静默写入路径。
...
- 必跑：`cd opencode-bridge && npm install && npm test`（109 个）+ `npm run typecheck`。
