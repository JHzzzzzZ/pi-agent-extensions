...
- `index.ts` — Pi 宿主接线：session_start 探测/拉起、`/opencode-bridge`（状态）、`/opencode-bridge-sync [port]`、`/opencode-bridge-restore`。
...
- 必跑：`cd opencode-bridge && npm install && npm test`（108 个）+ `npm run typecheck`。
