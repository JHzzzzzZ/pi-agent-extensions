...
- 必跑：node --experimental-strip-types --test src/extensions/provider-quota/index.test.ts（26 个测试；此目录无 package.json，npm test 跑不了）。仓库级：`npm run test:all`（本套件已登记，见 docs/tools/test-all.md）。
