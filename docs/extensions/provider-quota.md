...
> last verified @ ea3281a

## 职责与边界

footer 状态行显示当前 provider 的余额/额度（无 provider 前缀；智谱 `tokX% mcpY%(HH:mm)`、Go `X%/Y%/Z%(HH:mm)`、Kimi `5h7% m0%(HH:mm)`；段前缀由本地 `status-band.ts` 决定：最前段无前缀、其余段 `│ `）：session 启动即查、每 5 分钟轮询、切换 model 自动刷新、`/quota` 手动刷新。内置适配 openrouter/deepseek/chatanywhere/zhipu/opencode-go/kimi-coding。**不做**：内置列表外 provider（anthropic/openai 直连等）静默不显示；**不读环境变量取 API key**（凭据只认 auth.json）。

## 文件地图

- `index.ts` — 全部实现（单文件约 750 行，无 package.json）。`QUOTA_ENDPOINTS` 是加新 provider 的唯一入口（url / method / auth / parse 四件套）。
- `index.test.ts` — 38 个测试；纯解析函数 `parseZhipuQuotaLimit` / `parseOpencodeGoUsage` / `parseKimiCodingUsage` 显式导出且 now 由参数注入——固定时钟测试的关键设计。`STATUS_ID` 也导出供键带断言；写入边界（最前段无前缀 / 非最前段 `│ ` / 低带出现消失重渲染）用临时 HOME + 带 query 的动态 import 实例化真实扩展钩子验证。
- `status-band.ts` — 每插件一份的 footer 段前缀登记（`Symbol.for("pi.status-bar.bands.v1")` 进程共享表）；`writeBand` 是唯一写入口，改前缀规则只动此文件（五份拷贝同步）。
- provider id 经 `PROVIDER_ALIASES` 归一（glm/zai/bigmodel→zhipu，zen/opencode→opencode-go，kimi→kimi-coding），key 读取带 `AUTH_ID_FALLBACK` 回退链（kimi-coding↔kimi 双向覆盖）。
...
- kimi-coding（实测 2026-09-19，规格 docs/specs/provider-quota-kimi-coding.md）：只支持实测形态——`limits[]` 5h 计数窗（window.duration=300 TIME_UNIT_MINUTE，**detail 数值是字符串**，`used` 缺时由 `remaining` 反推）+ `usages.limit_5h/limit_month_total/limit_month_code` 的 `{used_ratio,reset_time}`；旧形态（`usage` 单对象/`data[]`）刻意不兼容。纳秒 ISO 由 Node ≥22 Date.parse 原生容忍（截断到 ms），无需手工截位。5h 窗匹配 duration 300min/5h，仅一条无匹配仍取该条、多条无匹配放弃计数兜底。**计数段（如 `40/100`）与 5h% 同窗冗余，#8 起不外显**——计数窗仅作内部兜底（5h% 缺失反推 / 5h 重置时间 / 达限判定，计数打满即 5h 达限）。used_ratio 是小数比例（超限 >1 如实显示 >100%），达限（ratio≥1 或计数打满）窗口的重置时间优先（5h > 月度，同 opencode-go 顺位），都未达限默认 5h。key 即 auth.json `kimi-coding` 条目的 `sk-kimi-`（Kimi Code 控制台 key，与开放平台 sk- key 不互通）。
- index.ts 顶部只允许 `import type`（类型擦除后零外部依赖），这是能用 `node --experimental-strip-types` 直接单测的前提；加任何运行时 import 都会破坏它。

## 改动清单

- 必跑：node --experimental-strip-types --test src/extensions/provider-quota/index.test.ts（38 个测试；此目录无 package.json，npm test 跑不了）。仓库级：`npm run test:all`（本套件已登记，见 docs/tools/test-all.md）。
