...
- 单文件 `index.ts`（约 400 行）+ `index.test.ts` + 带 `pi.extensions` 清单的 package.json + tsconfig；纯目录复制即可被 pi 加载。
...
2. `planDispatch` 纯决策三出口：help / blocked（create-new 撞已有文件且无 --yes）/ dispatch。
...
- 门控 fail-closed：create-new + 已有知识文件 + 无 `--yes` ⇒ 必须拒绝（`resolveCreateGate`，index.ts）；update 模式永不删文件，此契约不可放宽。
...
- 必跑：`cd deep-init && npm install && npm test`（32 个）+ `npm run typecheck`；缩进 2 空格。
