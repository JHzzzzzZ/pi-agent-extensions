...
| [agent-team](extensions/agent-team.md) | 多 agent 团队：leader 子进程调度成员子进程，同一会话多 run 并发（上限 3），报告 followUp 送达；team_stop 按 runId 中止；viewer 内 m 发消息直接对话（派单语义）；run 落盘/reconcile、budget 预算块、model 预检；裸 /team + 冒号子命令 `/team:list|:run|:status|:stop|:view|:clear|:doctor`（派单统一 `/team:run <名> <任务>`）；TUI 对照 pi-subagents 矩阵同步（docs/tui-sync.md） |
