...
- `agent-team/types.ts` — `TeamErrorCodes`（result union，上限同款：8 任务 / 4 并发 / 50KB / 8KB）；v1.17.0 新增 `RUN_NOT_TERMINAL`（续跑请求时父 run 仍在跑）与 `RESUME_UNAVAILABLE`（父 run 无 leader 会话镜像）。
